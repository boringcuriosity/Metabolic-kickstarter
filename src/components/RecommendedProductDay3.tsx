...
function Poster({ onClick }: { onClick?: () => void }) {
  return (
    <div 
      ...
      data-name="poster" 
      style={{ aspectRatio: '358/152' }}
      onClick={onClick}
    >
      <div 
        className="absolute inset-x-0 top-[8px] bottom-[8px] bg-center bg-cover bg-no-repeat" 
        data-name="cgm poster" 
        style={{ 
          backgroundImage: `url('https://hebbkx1anhila5yf.public.blob.vercel-storage.com/cgm%20poster-kzMeUOMPGgfIrA0Q8tNrPZiu3VVPXX.png')` 
        }} 
      />
      <NewTagDesignRed />
    </div>
...
interface RecommendedProductDay3Props {
  onClick?: () => void;
}

export function RecommendedProductDay3({ onClick }: RecommendedProductDay3Props) {
  return (
    <div className="relative w-full" data-name="recommended product day 3">

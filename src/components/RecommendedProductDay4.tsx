...
import svgPaths from "../imports/svg-0h8irjjmgy";
...
function Poster({ onClick }: { onClick?: () => void }) {
  return (
    <div 
      ...
      data-name="poster" 
      style={{ aspectRatio: '358/151' }}
      onClick={onClick}
    >
      <div 
        className="absolute inset-x-0 top-[8px] bottom-[8px] bg-center bg-cover bg-no-repeat" 
        data-name="melatonin poster" 
        style={{ 
          backgroundImage: `url('https://hebbkx1anhila5yf.public.blob.vercel-storage.com/red%20melatoin%20poster-jbLnjPC5rd7kERz0cAEKNhDGvgCRjH.png')` 
        }} 
      />
      <NewTagDesignRed />
    </div>
...
interface RecommendedProductDay4Props {
  onClick?: () => void;
}

export function RecommendedProductDay4({ onClick }: RecommendedProductDay4Props) {
  return (
    <div className="relative w-full" data-name="recommended product day 4">
